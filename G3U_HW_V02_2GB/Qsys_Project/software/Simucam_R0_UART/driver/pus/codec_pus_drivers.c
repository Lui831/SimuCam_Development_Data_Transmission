...
// Variable for holding app data lens of sent packages until now
static alt_u32 pSentTMsAppDataLen[CODEC_PUS_SEND_HEADER_STACK_SIZE] = {0};
static alt_u32 u32SentTMsitrt = 0;
...
void codec_pus_reset(t_codec_pus *pCodecPUS){
    ...
    // Resets the TMs sent counter
    codec_pus_get_tms_sent_num(pCodecPUS);

    // Resets the AppDataLen array
    for(alt_u32 u32Cont = 0; u32Cont < CODEC_PUS_SEND_HEADER_STACK_SIZE; u32Cont++) pSentTMsAppDataLen[u32Cont] = 0;
    u32SentTMsitrt = 0;

    // Resets the remasnescent irqs
...
void codec_pus_config(t_codec_pus *pCodecPUS){
    ...
    CODEC_PUS_REG_WRITE(pCodecPUS->u32CodecPUSBaseAddr, CODEC_PUS_RECV_DMA_MEM_OFFSET_REG_OFFSET, pCodecPUS->oCodecPUSRecvDmaConfig.u32DmaMemBaseAddr);
    CODEC_PUS_REG_WRITE(pCodecPUS->u32CodecPUSBaseAddr, CODEC_PUS_RECV_DMA_FIFO_SIZE_REG_OFFSET, pCodecPUS->oCodecPUSRecvDmaConfig.u32DmaFifoSize);

    // Configures the Send DMA registers
    CODEC_PUS_REG_WRITE(pCodecPUS->u32CodecPUSBaseAddr, CODEC_PUS_SEND_DMA_MEM_OFFSET_REG_OFFSET, pCodecPUS->oCodecPUSSendDmaConfig.u32DmaMemBaseAddr);
    CODEC_PUS_REG_WRITE(pCodecPUS->u32CodecPUSBaseAddr, CODEC_PUS_SEND_DMA_FIFO_SIZE_REG_OFFSET, pCodecPUS->oCodecPUSSendDmaConfig.u32DmaFifoSize);
...
void codec_pus_write_send_dma(t_codec_pus *pCodecPUS, alt_u8 *pAppData, alt_u32 u32AppDataLen){
    ...
    // If the data to be sent fits in the available space, writes it to the DMA FIFO
        ...
        for(alt_u32 count = 0; count < u32AppDataLen; count += 4){
            ...
            IOWR(pDMAFIFOPtr, 0, tempWord);
 
            pDMAFIFOPtr += 1; // Increments the pointer to the next word in the DMA FIFO

            if ((alt_u32) pDMAFIFOPtr >= pCodecPUS->oCodecPUSSendDmaConfig.u32DmaMemBaseAddr + pCodecPUS->oCodecPUSSendDmaConfig.u32DmaFifoSize)
            	pDMAFIFOPtr = (pCodecPUS->oCodecPUSSendDmaConfig.u32DmaMemBaseAddr);
    ...
    // Updates the current sent app data len
    pSentTMsAppDataLen[u32SentTMsitrt] = u32WrittenWords * 4;
    u32SentTMsitrt++;
...
void codec_pus_read_recv_dma(t_codec_pus *pCodecPUS, alt_u8 *pAppData, alt_u32 u32AppDataLen){
    ...
    // Declares the pointer to the current position in the DMA FIFO
    alt_u32* pDMAFIFOPtr = (alt_u32*) (pCodecPUS->oCodecPUSRecvDmaConfig.u32DmaMemBaseAddr + pCodecPUS->u32CurrentRecvDmaOffset);

    // Reads the data from the DMA FIFO, one word at a time
    for(count = 0; count < u32AppDataLen; count += 4){
		...
		if ((alt_u32) pDMAFIFOPtr >= pCodecPUS->oCodecPUSRecvDmaConfig.u32DmaMemBaseAddr + pCodecPUS->oCodecPUSRecvDmaConfig.u32DmaFifoSize)
			pDMAFIFOPtr = (pCodecPUS->oCodecPUSRecvDmaConfig.u32DmaMemBaseAddr);
...
void codec_pus_get_recv_tc_b(t_codec_pus *pCodecPUS, t_codec_pus_tc_recv_info *pRecvInfo, const alt_u32 u32TimeIncrement){
...
alt_u8 codec_pus_send_tm_nb(t_codec_pus *pCodecPUS, t_codec_pus_tm_send_info *pSendInfo){
	...
	u32TempVal |= (pSendInfo->oPkgPrimHdr.u16APID << CODEC_PUS_SEND_PKG_PRIM_HDR1_APID_OFFSET) & CODEC_PUS_SEND_PKG_PRIM_HDR1_APID_MASK;
	...
	u32TempVal |= (pSendInfo->oPkgSecHdr.u8SpacecraftTimeRefStatus << CODEC_PUS_SEND_PKG_SEC_HDR1_SPACECRAFT_TIME_REF_OFFSET) & CODEC_PUS_SEND_PKG_SEC_HDR1_SPACECRAFT_TIME_REF_MASK;
	...
	u32TempVal |= (pSendInfo->oPkgSecHdr.u16MessageTypeCounter << CODEC_PUS_SEND_PKG_SEC_HDR2_MSG_TYPE_COUNTER_OFFSET) & CODEC_PUS_SEND_PKG_SEC_HDR2_MSG_TYPE_COUNTER_MASK;
	u32TempVal |= (pSendInfo->oPkgSecHdr.u16DestID << CODEC_PUS_SEND_PKG_SEC_HDR2_DEST_ID_OFFSET) & CODEC_PUS_SEND_PKG_SEC_HDR2_DEST_ID_MASK;
...
void codec_pus_send_tm_b(t_codec_pus *pCodecPUS, t_codec_pus_tm_send_info *pSendInfo, alt_u32 u32TimeIncrement){
    ...
    // Verifies if the Codec PUS is ready to send a packet
    while(!codec_pus_rdy_for_send_tm(pCodecPUS)); // Sleeps for the specified time increment // TODO: check the Nios V sleep related function
...
alt_u8 codec_pus_get_tms_sent_num(t_codec_pus *pCodecPUS){
    ...
    // Creates a variable for accumulating app data sizes
    alt_u32 u32AppDataLenAcc = 0;
    ...
    // Accumulates the app_data of the global variable based on the currentTMsSent
    for(alt_u32 u32Cont = 0; u32Cont < (alt_u32) u8TMsSentDiff; u32Cont++){
    	u32AppDataLenAcc += pSentTMsAppDataLen[u32Cont];
    	pSentTMsAppDataLen[u32Cont] = 0;
    }

    alt_u32 u32Cont = 0;


    // Copies the last items in the array in the first parts
    for(u32Cont = 0; u32Cont < u32SentTMsitrt; u32Cont++) pSentTMsAppDataLen[u32Cont] = pSentTMsAppDataLen[u32Cont + u8TMsSentDiff];

    // Updates the itrt based on the sent packages
    u32SentTMsitrt -= u8TMsSentDiff;

    // Updates the current dma available space
    pCodecPUS->u32CurrentSendDmaAvailableSpace += u32AppDataLenAcc;
